...
これにより **AppleScript / Accessibility 権限への依存がトリガー経路から消え、macOS/Windows で同一の仕組みになる**(要実機検証: surface → EditTask 起動の結合部のみ。個別部品はすべて実証済み)。
...
|---|---|---|---|
| transport | MCU or Sora Surface(仮想 MIDI) | なし | MCU マッピング・ATOM の interpretCommand 実例確認済み。送信結合は未検証 |
| automation(マップ済みパラメータ) | MCU / Sora Surface(仮想 MIDI) | なし | 要検証 |
| コマンド実行(View 切替等) | Sora Surface → interpretCommand | なし | 部品ごとに実証済み・結合未検証 |
