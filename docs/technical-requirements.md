...
### 3.2 動作要件(対応 OS・クライアント非依存性・依存ポリシー)

Sora は複数の AI クライアントと複数の DAW から使われることを前提に、次を動作要件とする。

**対応 OS**

- Tool 層(sora バイナリ): **macOS / Windows / Linux** で動作すること。pure Rust 依存のみで構成し(§3.1)、CI は Linux 上でビルド・テスト・スキーマ検査を常時実行する。
- 仮想 MIDI(Phase 3+): macOS は IAC Driver(CoreMIDI)、Windows は loopMIDI、Linux は ALSA 仮想ポート。midir が三者を単一 API で抽象化する。OS 検出とセットアップガイドは `sora doctor` が提供する。
- DAW 統合(Phase 4)の対応 OS はアダプタごとに申告する(§11)。DAW 自体が動く OS(Studio One は macOS/Windows)を超えることはできない。

**AI クライアント非依存**

- Agent 層との接点は **(1) CLI(引数 + JSON stdin/stdout + 終了コード)、(2) `schemas/` の JSON Schema、(3) MCP サーバー(M4〜)** の3つのみとする。特定クライアントの独自機能(フック・スラッシュコマンド等)をコア経路の前提にしない。
- クライアント固有の指示ファイル(Claude Code の `CLAUDE.md`、Codex 等の `AGENTS.md`)は同一内容を指す薄い層として扱い、行動規範・ワークフローの実体は一元管理する。
- 実績: Claude Code と Codex の両方から同一の CLI・成果物スキーマで運用できることを確認済み(2026-07-05)。

**OS 固有機構への依存ポリシー**

- AppleScript / macOS Accessibility / Windows UI Automation などの **OS 固有の UI 自動化機構への依存は、コア経路では禁止**する。
- どうしても代替が無い操作は「**OS 限定のオプション capability**」として隔離し、DawCapabilities の申告で OS 依存であることを明示する。その capability が使えない環境では Generic(ファイルベース)経路へフォールバックする(§11.1)。
- 例: Studio One の write_clip における Import File ダイアログ自動入力は現状 macOS 依存のオプション扱い(§11.2.1)。コマンド実行・transport・automation は MIDI 経由の OS 非依存ルートを第一候補とする。
...
| 優先 | DAW | 経路 | 備考 |
|---|---|---|---|
| 参照実装(スコープ外) | REAPER | OSC(rosc)+ ReaScript ブリッジ | API が最も広く文書化されており、全ケイパビリティを検証できる。**2026-07-05 ユーザー判断で当面スコープ外**。アダプタ抽象の妥当性は Generic + Studio One + モックテストで確認する |
...
**検証済みのライブ編集ルート(Sora Bridge・Codex 検証 2026-07-05)**
1. Sora が JSON リクエストを `~/Documents/Studio One/SoraBridge/inbox/` へ書く
2. Sora Bridge 拡張が登録する `EditTask`(メニュー項目 `トラック > Apply Sora Bridge Inbox`)を実行する
...
初回検証ではステップ2のトリガーに AppleScript(メニュークリック)を使ったが、これは §3.2 の依存ポリシー(OS 固有 UI 自動化のコア経路禁止)に反する。**トリガーは次の MIDI 経由方式へ置き換える。**

**トリガーの OS 非依存化: Sora Surface(MIDI トリガー式コントロールサーフェス)**

Studio One のコントロールサーフェスは JS スクリプトで定義でき、**MIDI イベントだけで駆動され、スクリプトから `Host.GUI.Commands.interpretCommand` を呼べる**。これは推測ではなく、PreSonus 純正 ATOM デバイスのスクリプト(`Contents/devices/PreSonus/ATOM/ATOMComponent.js`)が実際にパッド MIDI から `interpretCommand("Transport", "Start")` 等を実行していることで裏付けられる(実バンドル内で確認済み)。

設計: Sora 専用のユーザーデバイス定義(`Sora Surface`)を `User Devices/` に設置し、仮想 MIDI ポート(midir で全 OS 対応)から特定 MIDI メッセージを受けたら
- 単発コマンド(transport 等)は surface スクリプトが直接 `interpretCommand` で実行
- 構造編集は surface スクリプトが Bridge の EditTask をコマンド起動(または inbox 処理を直接呼ぶ)し、inbox JSON を処理させる

これにより **AppleScript / Accessibility 権限への依存がトリガー経路から消え、macOS/Windows で同一の仕組みになる**。

**結合検証済み(2026-07-05・実機)**: Sora Surface(純 XML デバイス、`nativeCode="ControlSurfaceDevice"` + `<Global>` の `<Command>` マッピング。KX DAW Remote と同型)を仮想 MIDI ポートから駆動し、(a) Transport Start/Stop の発火(再生ヘッドの移動を目視確認)、(b) Bridge inbox の消化(`dump_command` の outbox 出力)を確認した。EditTask のコマンドカテゴリは `Track`(`TrackEdit` は存在しない)、Bridge ProgramService のコマンド `Sora / Apply Next Command` も登録されることを実機の `dump_command` で確認。**トリガー用の仮想 MIDI ポートは演奏プレビュー用と必ず分離する**(同居させるとキースイッチ音がコマンドとして解釈される。例: C#0 = 0x19 = Stop)。

**確定した否定的事実**
- `.song` / `.musicx` / `.envelopex` をディスク上で書き換えても、開いているドキュメントには**ホットリロードされない**(確認済み)。オフライン編集は「閉じて開き直す」前提の経路としてのみ有効。
- 完全バックグラウンド常駐(`UserService`/`ProgramService` によるポーリング)は Studio One 5 では動作せず。トリガー(メニュー実行 or MIDI イベント)が毎回必要。
- `Song > Import File` への直接パス引数渡しは未発見(`interpretCommand` のシグネチャにパス引数の口が無い)。ファイル選択ダイアログの自動入力で回避できることは検証済みだが、これは OS 固有 UI 自動化なので **macOS 限定のオプション capability に隔離**する(§3.2)。恒久解は拡張内(EditTask)からモデル API で直接クリップを生成する方式の R&D。
- UCNET(Studio One Remote のネットワークプロトコル)は proprietary で流用可能な OSS 実装なし → 不採用。`studioone://` URL スキームも任意コマンド注入には使えない見込み。

**各ケイパビリティの実装方針(§3.2 の依存ポリシー適用後)**
| capability | 第一候補ルート | OS 依存 | 状態 |
|---|---|---|---|
| transport | Sora Surface(仮想 MIDI) | なし | **実機検証済み(2026-07-05)** |
| automation(マップ済みパラメータ) | MCU / Sora Surface(仮想 MIDI) | なし | 要検証 |
| コマンド実行(View 切替等) | Sora Surface → interpretCommand | なし | **実機検証済み(2026-07-05)** |
| write_clip | MIDI 生成 → Bridge(EditTask)で Import File | ダイアログ自動入力部分のみ macOS 限定(オプション) | AppleScript 版は実機検証済み。拡張内モデル API での直接生成が恒久解(R&D) |
| render | Bridge でエクスポートコマンド + 出力監視 | ダイアログ自動化が必要なら OS 限定 | 未検証 |
| read | `.song`(ZIP+XML)のオフライン読解 or Bridge 経由ダンプ | なし | 部分検証 |

**初期セットアップ(`sora daw setup studio-one` として自動化する)**
1. Studio One 検出 → Sora Bridge 拡張 + Sora Surface デバイス定義のインストール/更新(冪等)→ `Extensions.settings` 有効化 → inbox/outbox 作成 → インストールレシート記録
2. ユーザー操作が必要なのは、**Studio One の再起動**(拡張導入時)と、Studio One 環境設定での **Sora Surface デバイスの有効化 + 仮想 MIDI ポートの割当**(初回のみ。doctor が手順を案内)。macOS 限定オプション(ダイアログ自動化)を使う場合のみ Accessibility 権限を追加で求める。
3. セットアップ後は、検証済みフローにユーザーの手動操作は不要

安全規約(§11.4)はこのルートにも適用する: セットアップは既存 `.song` に触れない、拡張は `sora.studioone.bridge` / `Sora Surface` のみを管理、アンインストール手段を提供、Studio One の再起動は明示承認制。

**結論**: `.song` ホットリロードと UCNET は行き止まり。**トリガーと transport/automation は Sora Surface / MCU(仮想 MIDI = OS 非依存)**、**構造編集は Sora Bridge 拡張(EditTask)**の2部品構成を採用する。AppleScript は「macOS 限定のオプション capability(ダイアログ自動化)」に隔離し、コア経路からは排除する。Codex 検証(2026-07-05)の成果物(拡張プロトタイプ・インストーラ・検証レポート)は `~/Documents/Codex/2026-07-05/cl/outputs/` にあり、M5 実装時の参照実装とする。
...
- **selection 非対応アダプタでの範囲指定(Note Selector)**: 選択状態を取得できないアダプタ(Studio One 5 を含む)では、ユーザーの自然言語の範囲指定(例: 「ベーストラックの第 5〜8 小節の C2 以下のノート」「Verse セクションのノート」)を Agent が構造化セレクタへ翻訳し、Tool 層がそれを決定論的に解決する。セレクタは `bars`(小節範囲)/ `section`(project-context.json の sections ラベル)/ `pitch_min`・`pitch_max`(音名境界)/ `note_indices` の AND 結合。トラックはファイル(=指示語解決で確定した素材)が担う。合致 0 件は黙って無視せずエラーで返す。

### 11.4 書き込み操作の安全規約(Level 4+)

1. **書き込み前スナップショット**: `write_clip` / `write_automation` は実行前に対象の現状態を `versions/daw-backups/` に保存する。粒度はアダプタの read 能力に応じて最良のものを選ぶ(トラック状態を読めるアダプタはトラック状態、読めないアダプタは最低限「最後に保存されたプロジェクトファイルのコピー + 書き込み内容そのもの」)。それすら取得できない場合は書き込みを拒否する(undo 不能な操作をしない)。
...
| 3 | ~~**対象 DAW の確定**~~(解決済み 2026-07-05): 実 DAW は Studio One 5。公式公開 API は無いが、実現ルートを実機検証で確定(§11.2.1) | — | トリガー/transport/automation は Sora Surface・MCU(仮想 MIDI = OS 非依存)、構造編集は Sora Bridge 拡張(EditTask)。AppleScript は macOS 限定オプション(ダイアログ自動化)に隔離。`.song` ホットリロードと UCNET は行き止まりと確認。Generic アダプタは常設フォールバック |
